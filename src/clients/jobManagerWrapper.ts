...
export class JobManagerWrapper extends JobManagerClient {
  ...
  public async completeTask(task: ITaskResponse<ITaskParameters>): Promise<void> {
    if (task.jobId == undefined) {
      throw new AppError('', httpStatus.INTERNAL_SERVER_ERROR, `Task ${task.id} doesn't contain jobId`, false);
    }
  ...
  public async progressJob(jobId: string | undefined): Promise<boolean> {
    ...
    if (jobId == undefined) {
      throw new AppError('', httpStatus.INTERNAL_SERVER_ERROR, `jobId is undefined`, false);
    }
    try {
      const job  = await this.getJob(jobId);
      if(job == undefined) {
      throw new AppError('', httpStatus.INTERNAL_SERVER_ERROR, `Job ${jobId} doesn't exists`, false);
      }
